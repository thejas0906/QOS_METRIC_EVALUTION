...
class CostPerformanceUtilityEngine:
    ...
    def set_tradeoff_parameters(self, alpha: float, beta: float) -> None:
        """Updates alpha and beta tradeoff weights dynamically."""
        if alpha < 0 or beta < 0:
            raise ValueError(f"Tradeoff weights must be non-negative: alpha={alpha}, beta={beta}")
        self.alpha = float(alpha)
        self.beta = float(beta)
    ...
    def compute_utility(
        ...
        """
        Computes the utility matrix:
            Utility(u, s) = alpha * QoS_Score(u, s) - beta * Cost(s)
