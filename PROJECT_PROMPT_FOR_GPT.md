...
### 2.1 Team A: Biased Matrix Factorization (BiasedMF)
...
### 2.2 Team B: Bipartite GraphSAGE & Multi-Task QoS Decoder
...
### 2.3 Team B: Cost-Performance Utility Formulation
...
## 3. Empirical Results Generated by the Simulation

### 3.1 Prediction Accuracy (RMSE & MAE)
| QoS Attribute | Team A (BiasedMF) RMSE | Team A (BiasedMF) MAE | Team B (GraphSAGE) RMSE | Team B (GraphSAGE) MAE |
| :--- | :---: | :---: | :---: | :---: |
| Availability | 0.2052 | 0.1620 | **0.1209** | **0.1032** |
| Latency (ms) | **127.8594** | **95.4314** | 377.2965 | 341.9292 |
| Reliability | 0.2061 | 0.1627 | **0.1835** | **0.1712** |
| Response Time (s) | **0.4611** | **0.3461** | 0.7152 | 0.5353 |
| Throughput (kbps) | **873.9087** | **645.8262** | 2057.3523 | 1827.9711 |

### 3.2 Recommendation Quality (Top-$K$ Ranking)
...
### 3.3 Economic Performance (Team B Pareto Advantage)
- **Average Invocation Cost (Team B Cost-Aware)**: **$0.1466** per call
- **Average Invocation Cost (Baseline QoS-Only)**: **$0.3299** per call
- **Cost Savings**: **55.55% reduction**
- **Cost-Performance Ratio**: **3.6520**
- **Mean Utility Score**: **0.3190**

### 3.4 Cost-Performance Trade-off Grid ($\alpha + \beta = 1.0$)
...
## 4. Generated Publication LaTeX Tables (`results/metrics/latex_tables.tex`)
...
## 5. Key Implementation Code Modules

### 5.1 Bipartite GraphSAGE Module (`src/team_b_gnn/models/graphsage_module.py`)
```python
import torch
...
### 5.2 Multi-Task QoS Edge Decoder (`src/team_b_gnn/models/qos_prediction_decoder.py`)
```python
import torch
import torch.nn as nn
import torch.nn.functional as F
...
### 5.3 Cost-Performance Utility Engine (`src/team_b_gnn/utility_engine.py`)
```python
import numpy as np
...
### 5.4 Biased Matrix Factorization (`src/team_a_matrix/models/matrix_factorization.py`)
```python
import numpy as np
...
# 2. Execute Team A Matrix Factorization experiments
python experiments/run_team_a.py --synthetic --epochs 80

# 3. Execute Team B GraphSAGE + Cost-Performance experiments
python experiments/run_team_b.py --synthetic --epochs 80 --alpha 0.7 --beta 0.3

# 4. Generate Comparative Tables & LaTeX output
python experiments/run_comparison.py

# 5. Generate Cost-Performance Pareto Frontier Sweep
